...
fluorescence.lines.directory <- "data/LineDefinitions.rdata"
...
elementFrame <- function(data, range.table = NULL, elements, cores = 1,
                         worker_pkgs = NULL, extra_exports = NULL) {
  stopifnot(length(elements) > 0)
  
  .worker <- function(x) elementGrab(element.line = x, data = data, range.table = range.table)
  
  if (cores > 1L) {
    n  <- min(as.integer(cores), length(elements))
    cl <- parallel::makeCluster(n, type = "PSOCK")
    on.exit(try(parallel::stopCluster(cl), silent = TRUE), add = TRUE)
    
    # load pkgs on workers (if elementGrab needs them)
    if (!is.null(worker_pkgs) && length(worker_pkgs)) {
      parallel::clusterExport(cl, "worker_pkgs", envir = environment())
      parallel::clusterEvalQ(cl, {
        suppressPackageStartupMessages({
          for (p in worker_pkgs) if (requireNamespace(p, quietly = TRUE))
    ...
    # export objects used by .worker
    to_export <- unique(c(".worker", "elementGrab", "data", "range.table", extra_exports))
    to_export <- to_export[nzchar(to_export)]
    if (length(to_export)) parallel::clusterExport(cl, to_export, envir = environment())
    
    spectra.line.list <- pbapply::pblapply(elements, .worker, cl = cl)
    
  } else {
    # serial path still uses pbapply (works on all OSes)
    spectra.line.list <- pbapply::pblapply(elements, .worker)  # no 'cl' = serial
  }
  ...
  spectra.line.frame <- spectra.line.frame[order(as.character(spectra.line.frame$Spectrum)), , drop = FALSE]
  spectra.line.frame$Spectrum <- gsub("\\.(pdz|csv|CSV|spt|mca|spx)$", "", spectra.line.frame$Spectrum)
  spectra.line.frame
}
...
wideElementFrame <- function(data, elements, range.table = NULL, cores = 1,
                             worker_pkgs = NULL, extra_exports = NULL) {
  stopifnot(length(elements) > 0)
  
  # one task: must return at least 2 cols: Spectrum + value
  .worker <- function(el) {
    df <- wideElementGrab(element.line = el, data = data, range.table = range.table)
    df <- as.data.frame(df, stringsAsFactors = FALSE)
    if (!"Spectrum" %in% names(df)) names(df)[1] <- "Spectrum"
    if (ncol(df) < 2) stop(sprintf("wideElementGrab('%s') returned <2 columns", el))
    df[, 1:2, drop = FALSE]
  }
  
  # serial path still uses pbapply (no forking anywhere)
  if (cores <= 1L) {
    spectra.line.list <- pbapply::pblapply(elements, .worker)
  } else {
    n  <- min(as.integer(cores), length(elements))
    cl <- parallel::makeCluster(n, type = "PSOCK")
    on.exit(try(parallel::stopCluster(cl), silent = TRUE), add = TRUE)
    
    # load packages on workers if needed
    if (!is.null(worker_pkgs) && length(worker_pkgs)) {
      parallel::clusterExport(cl, "worker_pkgs", envir = environment())
      parallel::clusterEvalQ(cl, {
        suppressPackageStartupMessages({
    ...
    # export objects used by .worker
    to_export <- unique(c(".worker", "wideElementGrab", "data", "range.table", extra_exports))
    to_export <- to_export[!is.na(to_export) & nzchar(to_export)]
    if (length(to_export)) parallel::clusterExport(cl, to_export, envir = environment())
    
    spectra.line.list <- pbapply::pblapply(elements, .worker, cl = cl)
  }
  
  # --- reshape like the original ---
  # Expect each list item: columns c("Spectrum", <value>)
  element.count.list <- lapply(spectra.line.list, `[`, 2)           # 2nd col only
  spectra.line.vector <- as.numeric(unlist(element.count.list))
  
  n_rows <- nrow(spectra.line.list[[1]])
  n_cols <- length(elements)
  dim(spectra.line.vector) <- c(n_rows, n_cols)
  
  spectra.line.frame <- data.frame(
    Spectrum = spectra.line.list[[1]][[1]],
    spectra.line.vector,
    stringsAsFactors = FALSE
  )
  colnames(spectra.line.frame) <- c("Spectrum", elements)
  
  # tidy Spectrum as in original
  spectra.line.frame <- spectra.line.frame[order(as.character(spectra.line.frame$Spectrum)), , drop = FALSE]
  spectra.line.frame$Spectrum <- gsub("\\.(pdz|csv|CSV|spt|mca|spx)$", "", spectra.line.frame$Spectrum)
...
spectra_gls_deconvolute <- function(
    spectra_frame, baseline = TRUE, cores = 1
) {
  # normalize
  ...
  spectra_frame <- spectra_frame[complete.cases(spectra_frame), , drop = FALSE]
  
  # split
  spectra_list <- split(spectra_frame, spectra_frame$Spectrum)
  if (!length(spectra_list)) {
    empty <- list(Spectra = data.frame(), Areas = data.frame())
    return(if (isTRUE(baseline)) c(empty, list(Baseline = data.frame())) else empty)
  }
  
  if (cores > 1L) {
    n  <- min(cores, length(spectra_list))
    cl <- parallel::makeCluster(n, type = "PSOCK")
    on.exit(try(parallel::stopCluster(cl), silent = TRUE), add = TRUE)
    
    # Load needed pkgs directly on workers (no 'libs' var needed)
    parallel::clusterEvalQ(cl, {
      suppressPackageStartupMessages({
        library(data.table); library(xrftools); library(tidyverse)
      })
      NULL
    })
    
    # Export only helpers that actually exist in *this* function's env
    helpers <- c(
      "deconvolute_complete",
      "as_tibble","tibble_convert",
      ...
      "atomic_order_vector","atomic_order",
      "fluorescence.lines","line_strip"
    )
    helpers <- helpers[helpers %in% ls(envir = environment()) | vapply(helpers, exists, FALSE, inherits = TRUE)]
    if (length(helpers)) {
      parallel::clusterExport(cl, varlist = helpers, envir = environment())
    }
    
    new_spectra_list <- pbapply::pblapply(spectra_list, deconvolute_complete, cl = cl)
  } else {
    new_spectra_list <- pbapply::pblapply(spectra_list, deconvolute_complete)
  }
  
  # unpack + bind
  only_spectra_list <- lapply(new_spectra_list, `[[`, "Spectra")
  only_areas_list   <- lapply(new_spectra_list, `[[`, "Areas")
  new_spectra_frame <- as.data.frame(data.table::rbindlist(only_spectra_list, use.names = TRUE, fill = TRUE))
  new_area_frame    <- as.data.frame(data.table::rbindlist(only_areas_list,   use.names = TRUE, fill = TRUE))
  
  if (!isTRUE(baseline)) return(list(Spectra = new_spectra_frame, Areas = new_area_frame))
  
  only_background_list <- lapply(new_spectra_list, `[[`, "Baseline")
  new_baseline_frame   <- as.data.frame(data.table::rbindlist(only_background_list, use.names = TRUE, fill = TRUE))
  
  if (nrow(new_baseline_frame)) {
    bl_sum <- aggregate(CPS ~ Spectrum, data = new_baseline_frame[, c("Spectrum","CPS")], FUN = sum)
    new_area_frame <- merge(new_area_frame, bl_sum, by = "Spectrum", all.x = TRUE)
    names(new_area_frame)[names(new_area_frame) == "CPS"] <- "Baseline"
  } else {
    new_area_frame$Baseline <- NA_real_
  }
  
  list(Spectra = new_spectra_frame, Areas = new_area_frame, Baseline = new_baseline_frame)
}
